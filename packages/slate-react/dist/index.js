...
var useChildren = function useChildren(props) {
  ...
  for (var i = 0; i < node.children.length; i++) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var dec = _step.value;
        var d = slate.Range.intersection(toAbsoluteRange(dec), range);
...
var toAbsoluteRange = function toAbsoluteRange(range) {
  var absAnchor = toAbsolutePoint(range.anchor);
  var absFocus = toAbsolutePoint(range.focus);
...
var toAbsolutePoint = function toAbsolutePoint(point) {
  return point.basePath ? {
    path: [].concat(_toConsumableArray(point.basePath), _toConsumableArray(point.path)),
    offset: point.offset
  } : point;
};
